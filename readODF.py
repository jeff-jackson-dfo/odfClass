...
def read_odf(in_file):
    ...
    # Find the data line separator, there must be one and only one. Otherwise raise and
    # exception indicating if there are too many or too few.
    data_lines = [i for i, line in enumerate(lines) if re.match(DATA_LINE, line)]

    if len(data_lines) <= 0:
        raise Exception(
            " -- The input odf file does NOT have a separated beginning line for data section such as: '" + DATA_LINE + "'")
    ...
    # S is the returned structure containing the parsed file information.
    S = OrderedDict()
    ...
    for idxLine in range(0, data_lines[0] + 1):
        ...
        if line in parameter_keys or idxLine >= data_lines[0]:
            ...
            if current_parameter_name is not None:
                if current_parameter_name in S:
                    ...
                    if type(S[current_parameter_name]) is not list:
                        tmp = S[current_parameter_name]
                        S[current_parameter_name] = list()
                        S[current_parameter_name].append(tmp)

                    # Append the new parameter to the parameter list.
                    S[current_parameter_name].append(current_parameter)
                else:
                    # Add the new parameter to the return structure.
                    S.update({current_parameter_name: current_parameter})
    ...
    S.update({DATA: data_array})

    S.update({"INPUT_FILE": in_file})
